...
实现基线 HEAD：`10cb0e1`；本卡只描述 `LAB_ONLY` 实验和固定开发 ERP 只读证据，不授予业务 Runtime 或 ERP 写入权限。旧阻塞 artifact 保留为历史记录，当前结论只引用下列新证据。
...
- synthetic live 冻结：[phase11-benchmark-synthetic-live-glm-10cb0e1-r3.json](phase11-benchmark-synthetic-live-glm-10cb0e1-r3.json)，SHA-256 `98fd39dbeabae9a9e6e1ad496f7e6f9dfe036ac86eef8bd6557645b45e7e838b`；45 主 trial、78 fault、安全记录，`test_double_methods=[]`。
- 真实 ERP live：[phase11-benchmark-erp-readonly-live-glm-10cb0e1-r3.json](phase11-benchmark-erp-readonly-live-glm-10cb0e1-r3.json)，SHA-256 `bc18ee8ac51db64c2ea9e51365ed9007a10576f38527fef357e7176a51bc945a`；API/Web `MATCHED=3/3`，GUI `SUCCEEDED=2/3`、一次 `FAILED/RESPONSE_CONTENT_MISSING`，before/after 字段、revision、时间和 digest 已落盘。
- 当前视觉边界：[phase11-erp-visual-boundary-live-glm-10cb0e1.json](phase11-erp-visual-boundary-live-glm-10cb0e1.json)；脱敏 `READY`、任务字段保留、账号和导航隐藏、业务写入为 0。
...
| DOM | `LAB_ONLY CANDIDATE` | 结构稳定、临时引用唯一、模型在预算内 | live `glm-5.3-flash` 有成功轨迹；冻结批次 6/9 正确，失败保留，不宣称生产稳定率 |
