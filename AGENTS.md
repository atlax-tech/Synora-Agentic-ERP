...
每当触发项目提问、阶段复盘或导师答疑环节时，必须先定位并读取 `docs/learning-notes/README.md`，按其中的命名、事实口径、回答结构和原文记录规则执行；随后将用户问题原文、Agent 的证据化回答、结论、未决项和复习状态落库到对应阶段学习笔记。对外面试表达默认业务价值优先，不主动暴露仓库学习定位、学习属性或实验分层；内部事实只作 Agent 校验依据，不在笔记中额外展开定位说明。纠偏或权威事实发生变化时，直接覆盖已经失效的回答，保留原始问题，不把旧答案继续作为当前口径。学习笔记必须通过 `git ls-files` 确认已追踪；若本地 exclude 仍忽略目录，使用 `git add -f` 纳入 Git，禁止让学习证据停留在未追踪状态。
