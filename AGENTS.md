...
- 用户在计划模式说“开始推进 Phase X”“开始完成阶段 X”或“开始推进下个阶段”时，Codex 必须自动按 `docs/PLAN.md` 核对阶段状态并形成阶段细化计划；计划获准并进入执行模式后，自动调用全局 `codex-with-chatgpt`，不得要求用户再次提示 Skill、拆解方式或协作步骤。本仓库把这些阶段指令视为该 Skill 的隐式触发条件，但不改变官方状态流、权限边界或恢复流程。
...
- **注意**：ChatGPT 给出的结论也不能完全无脑采信，必须验证给出的执行步骤是否与现状匹配。如果执行过程中发现任何问题需继续与gpt进行深度对抗性探讨，直到结论唯一。review始终保持第一性原则，不将任何结论当作预设默认正确。
...
- 每个工作区复用一个长期 C2C 连接器和 ChatGPT 会话。只有用户明确授权并由 Codex 推送后，ChatGPT 才可对远端 GitHub 仓库或 PR 做补充性只读 Review；不得评论、批准、请求修改、提交、推送、修改、合并或关闭远端对象。
- **注意**：当gpt的回复明显出现幻觉/方向偏移/信息丢失/性能缓慢/与实际情况不符等情况代表上下文窗口过长需要触发C2C的切换到新窗口操作，做好handoff交接工作，不要将始终限制在单独一个长对话窗口。
