...
一个增量只能交付一个可独立解释、测试、审查和回滚的结果。禁止把相邻功能、顺手重构、文档整理或依赖升级混入同一个增量。

### 4.1 开始前：Context Receipt

执行 Agent 在编辑前必须用大白话写明：

- 当前阶段和步骤编号；
- 对应的 PRD 功能编号、验收条件和上一步输出；
- 为什么现在做这一步；
- `CONFIRMED`、`INFERRED`、`UNRESOLVED`、`CONFLICTED` 事实及来源；
- 允许和禁止修改的文件范围；
- 正常、异常、边界、恢复和安全风险；
- 将运行的自动化检查和可重复人工验收。

能从仓库、固定上游源码、官方测试或运行环境查到的信息必须先查，不能把可发现事实当问题抛给用户。

### 4.2 Execute 角色

- 只实现 Context Receipt 中的单一结果；
- 同时写最小充分的自动化测试和本次中文开发日志草稿；
- 不修改上游，不包含用户临时文件，不扩大授权范围；
- 不以 Mock、fixture、占位页面或模型演示替代真实企业集成完成度；测试要求明确允许的 test double 除外；
- 报告实际改动、命令、退出状态、失败、未运行检查和人工验收步骤；
- 不得为自己的实现返回 review `PASS`。

### 4.3 Test 角色

Test 角色必须使用独立 Agent 上下文，不相信 Execute 的结论，也不修复实现。输入必须包含原始需求、允许的文件边界、完整 diff、运行环境和测试命令。

至少挑战：

- 正常业务路径；
- 无效输入、无权限、对象不存在和上游校验失败；
- 空值、分页、并发、重复请求和状态漂移；
- 超时、响应丢失、恢复、对账和人工接管；
- Prompt Injection、越权工具、敏感信息和直接数据库/内部 import；
- ERP 最终状态，而不只是 Agent 文本。

保留命令、退出码、日志或产物和环境限制，返回且只返回一个结论：`PASS`、`FAIL` 或 `BLOCKED`。

### 4.4 Review 角色

Review 角色必须使用独立 Agent 上下文，读取原始任务、事实权威、完整 diff 和独立测试原始证据，不接受 Execute 的摘要作为证明。

审查范围包括：需求完整性、阶段边界、代码正确性、回归、权限、身份、数据一致性、审计、故障恢复、依赖方向、可维护性和测试缺口。Review 必须主动提出至少一个针对当前风险的对抗场景，并检查开发日志是否能让非技术用户看懂。

Review 最终只返回：`PASS`、`CHANGES_REQUIRED` 或 `BLOCKED`，每个问题必须有文件级证据和严重度。复杂度检查不得代替正确性和安全审查。

### 4.5 修正、提交与继续

- `FAIL` 或 `CHANGES_REQUIRED`：Execute 修正后，重新运行完整的独立 Test 和 Review；旧结论自动失效；
- `BLOCKED`：立即停止并按第 8 节交给用户；
- 只有 Test=`PASS` 且 Review=`PASS` 才能提交；
- 提交必须包含实现、测试、必要文档和同一增量的中文开发日志；
- 使用一个小而完整的 Conventional Commit，不推送、不发布、不改写历史；
- 提交后继续当前阶段的下一个未完成步骤；阶段出口通过后停止。
...
| 每个代码增量的独立 Review | `ponytail-review` | 正确性和安全审查完成后单独检查当前 diff 的过度设计；只报告可删除复杂度，不自动修改，也不替代主审查。 |
| 每个阶段出口 | `ponytail-audit`、`ponytail-debt` | 对全仓做只读复杂度审计并收集明确标注的 Ponytail 延期项；结果进入阶段报告，不凭审计结果自动删代码。 |
| Harness 或权威文档需要同步 | `harness-update` | 先做只读 drift 检测并给出带项目编号的文件级 proposal；只有用户明确批准这些项目后才能写入、刷新哈希和增加开发日志。 |
...
每个子里程碑必须单独执行“源码证据 → 契约与审批 → 实现 → 幂等与恢复 → UI → 真实 ERP 测试 → 独立对抗审查”，不得合并提交：
