...
- 代表题 `G1` 为真实 `qwen3:8b` Coach `PASS`（HTTP 200、`ANSWERED`、服务端重建答案、`ERP_FACT`/`LIVE_ERP`、`tools=[]`、prompt tokens > 0、MR/PO 锚点不变、无 Secret）。正式固定题序 `G1,G2,G3,G4,C1,C2,C3,S1,S2,S3,U1,U2`，每题 `attempt=1`，`12/12 PASS`；grounding `4/4`、citation `3/3`（positive `2/2`、safe refusal `1/1`）、refusal/security `3/3`、usefulness `2/2`。
- Provider 证据：10 个 eligible case 均有真实 usage，`provider_request_count=10`；S1/S2 是安全旁路。`mock_substitution=false`、`erp_business_zero_write=true`、`provider_tools_empty=true`、`repo_status_unchanged=true`、`secret_leak=false`、`selective_rerun=false`，MR/PO/库存业务锚点前后一致。
- 角色浏览器证据 `output/playwright/phase8-role-acceptance-562fc42.json` 为脱敏绑定 artifact：绑定当前 manifest/backend 证据，同时保留历史捕获 HEAD `20ecff4`；明确 `BIND_EXISTING_BROWSER_CAPTURE`，不把历史截图宣称为新执行。Buyer、System Manager、Viewer 的权限/零写入观察均通过。
- L3：`make format-check`、`make lint`、`make type`、`make unit` 通过（`676 passed, 2 warnings`）；真实 Bench app-test `210 tests ... OK`；compileall、`git diff --check`、固定 Frappe/ERPNext SHA 洁净检查通过。直接等价 Bench 命令已运行；因用户无关 dirty 的 `env/dev/scripts/dev/env.sh` 等文件，本轮没有把未运行的 `make integration` 写成通过。
- 独立审查：只读 Codex 审查角色第二轮必须为 `PASS` 才能关闭本阶段；审查输入为当前代码、上述真实证据、浏览器绑定、权限/零写入和 Provider 成本/超时边界。Ponytail review/audit 为最小充分实现；现有一条带真实触发条件的固定上限债务保留，未新增无触发债务。
- Rubric：D1 `4`、D2 `4`、D3 `3`、D4 `3`、D5 `4`、D6 `4`、D7 `4`、D8 `4`、D9 `3`，合计 `33/36`、平均 `3.67`；要求维度均至少 `3`，无 P0/P1。P2 仅为用户无关 dirty 环境文件的后续清理：owner 为项目维护者，Phase 9 入口先确认并恢复整洁后复验 `make integration`，不阻塞本阶段出口。
