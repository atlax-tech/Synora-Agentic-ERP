...
- **T08 / P8.5**：T08.1 内部 Runtime transport foundation 已在 `d6ab39c` 实现；新增 token 保护的 `POST /coach/answer`，Runtime 自己绑定当前 MR/PO Gateway、固定内部 curated FTS5（最多 5 条），并保持 Provider `tools=[]`。T08.2/Frappe adapter 以及后续 T08 Desk New Run、Runs、MR/PO 入口、Memory 原生审核页面、真实浏览器和 BYOK 验收仍未开始，待 T08.1 正式独立只读安全/diff Review 后再授权。
