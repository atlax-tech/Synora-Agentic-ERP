...
- 限制：开发 site 为单语言（en）、单会计年度环境；跨公司已验证（第二公司 fixture），但未覆盖「同一用户无公司 B 权限时的权限拒绝」组合（该组合由 security.py 的 initiator 权限重检保证，属 Phase 4 真实写场景再充分展开）；E2E 超时场景验证的是客户端 deadline，服务端 5s 工具超时已在 P2.4 集成测试覆盖。
...
- 被拒绝/延期：未引入 LangGraph（P3.6 spike 前不采用）；未引入任何 ORM 直连/任意 URL/任意工具路径；Runtime 保持零 ERP 依赖。
...
- **Ponytail 延期项（明确标注，不自动删除）**：
  ...
  - P3 `p26_e2e.py:258-267` CROSS_COMPANY 带 warehouse scope，company 过滤被仓库先行排除，建议后续补 warehouse=None 的 run 单独验证 company 过滤。
...
# B. 服务端集成：make integration（Ran 23 tests OK）
# C. 真实 HTTP 端到端：
...
## 8. 下一阶段：Phase 3（只读采购 Agent）尚未开始
