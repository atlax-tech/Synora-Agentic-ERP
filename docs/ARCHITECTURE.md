...
Status: `CONFIRMED` target architecture; Phase 0–4 components are implemented. Phase 5 durable workflow is implemented and its exit is `PASS` after the evidence recorded in ADR-0006 and the Harness baseline synchronization. Phase 6 governed MR/PO Draft implementation is complete and its exit is `COMPLETED / PASS` after real ERP, fault, browser, Harness, independent Test, and adversarial Review evidence. Phase 7 Prompt/Context/Skills implementation and exit evidence are complete with status `COMPLETED / PASS`; its Prompt/Context/Skill metadata remains non-authorizing, Frappe remains authoritative, and no ERP write capability was added. Phase 8 Memory/RAG/Contextual ERP Coach is `COMPLETED / PASS / READY FOR THE NEXT PHASE` (2026-09-03). Phase 9 implementation, protocol, real business acceptance, L3 evidence, independent adversarial review, and README/Harness synchronization are complete as of 2026-09-04 with status `COMPLETED / PASS / READY FOR THE NEXT PHASE`. Phase 10 is `COMPLETED / PASS / READY FOR THE NEXT PHASE` as of 2026-09-11 for the fixed isolated development ERP path; its governed P2P write boundary, real ERP evidence, failure recovery, browser acceptance, and protected documentation synchronization are recorded in the Phase 10 final report and manifest. This does not claim production deployment or customer adoption.
...
- The Phase 8 T03 slice has a Frappe-managed `Synora Memory Record` lifecycle boundary with server-side source/scope rechecks, race-safe dedupe, CAS review and correction transitions, tombstone deletion, permission-safe visible recall, and a native Desk queue. Retrieval indexing and Coach runtime use it only through the later, separately validated T04/T07 boundaries.
...
- The Phase 8 T06 slice exposes provider-neutral, strict current Material Request/Purchase Order context contracts and Frappe-permissioned read tools. Live ERP facts are scoped to the Run's company/warehouse and retain status, quantities, and modification timestamps; the tools remain service-selected and are not exposed as Provider schemas or model choices.
- The Phase 8 T07 slice validates Provider output and signs only post-validation claim packages with a domain-separated HMAC derived from the existing `SYNORA_RUNTIME_TOKEN`; Frappe independently verifies the signature, Run/correlation/company/warehouse scope, source revision, and live citation before persisting an immutable `Synora Coach Claim`. ERPNext/Frappe remains the live business-fact authority; retrieval and Memory remain `UNTRUSTED` evidence, Runtime-local SQLite remains `LAB_ONLY`, and the Provider receives `tools=[]` and cannot select current ERP tools.
...
Phase 4 establishes the execution-kernel baseline. Phase 5 establishes durable workflow. Phase 6 implements the first governed MR/PO Draft writes behind identity, permission, policy, approval, snapshot, digest, idempotency, read-back, Receipt, and reconciliation gates; real ERP and process-fault evidence, Buyer browser evidence, Harness checks, independent Test, and adversarial Review all passed. Phase 10 extends the same boundary through PO Submit, Purchase Receipt, Purchase Invoice, and Payment Entry on the fixed isolated development ERP, with independent approval, current-state rechecks, reservation/Receipt, ERP read-back, and process recovery evidence. Stricter or unverifiable enterprise Workflow mappings remain fail-closed, and the evidence does not claim production deployment.
...
Candidate roles are Procurement Planner, Policy/Compliance Reviewer, ERP Coach, and Reconciliation Agent. Multi-Agent implementation requires evaluation evidence for context isolation, independent review, permission separation, or useful parallelism. Phase 9 evidence adopted the bounded Planner → Policy/Risk Reviewer path using GLM `assist/glm-5.3-flash` under the quality-first profile: multi quality did not regress against the same-model single arm, valid/recovery quality improved, p95 remained controlled, and security stayed at 100%; token usage remains an audit metric. Coach remains the independent Phase 8 entry point, Reconciliation remains exception-triggered, and free-form agent swarms are prohibited. MCP, A2A, and ANP remain protocol-lab boundaries and do not receive ERP Gateway capability or Provider credentials.
