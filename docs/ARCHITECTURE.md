...
- The Phase 8 T03 slice now has a Frappe-managed `Synora Memory Record` review boundary with server-side scope rechecks, CAS review transitions, and a native Desk queue; it does not yet represent the complete Memory lifecycle or visible Coach recall.
