...
Synora 是“Agent 开发岗位学习仓库 + 真实 ERP 实践载体”：它面向学习者系统训练 Agent 执行、工具、工作流、上下文、记忆、协作、评测与运维能力，并在 ERPNext 真实采购业务中完成受权限约束的分析、规划、审批、执行、核对和审计。
...
- 在同一仓库内用“原理→最小实验→开源对照→Synora 业务魔改→测试/Trace→取舍→面试追问”形成可运行的学习证据。
- 形成可追溯、可复跑、可用于技术面试深入讲解的工程证据。

### 1.5 学习、业务与项目价值

- **学习价值**：通过可运行代码、失败轨迹、同任务对照、取舍记录和面试追问，形成可验证的 Agent 开发岗位能力。
...
- 不为商业化最小范围删除 Agent 岗位必需的学习主题；不适合业务主线的技术进入教学实验层并记录取舍。
...
| Agent 开发学习者 | 需要从执行循环到企业治理的真实经验 | 只会调框架 API，难以解释停止、恢复、记忆、协作和取舍 | 运行实验、完成 Assignment、对比 Trace、记录采用/拒绝证据 |
...
**阶段**：Phase 8（有限交付）；完整 P2P 写闭环仍由 Phase 10 负责。

**功能描述**：基于当前 Material Request / Purchase Order 页面、服务端重新鉴权的只读上下文、已验证 ERPNext 知识和模拟 SOP，提供来源可追踪、逐条带引用的解释。
...
**每个主题的必备证据**：原理说明、最小实验、开源源码对照、Synora 场景转译、成功/失败 Trace、同任务评测、Adoption Card 与面试追问。

**边界**：实验代码不得拿到生产凭证、绕过 Control Plane 或冒充已交付业务能力；不进入业务主线的技术仍保留可运行学习证据和拒绝理由。
...
- 首个桌面版本支持 Chrome、Edge、Firefox、Safari 最近两个主要版本，最小视口宽度 1280px，并以 WCAG 2.1 AA 作为键盘、可见焦点和非颜色状态提示基线；该范围由 P3.1 决策包于 2026-08-25 批准，详细约束见 `docs/DESIGN.md`。
- 移动端和离线模式不在当前范围。
...
- 每个阶段至少有一个用户完成或记录阻塞的 Assignment，未作答项标记“待练习”。
- 每个学习主题可运行、可复跑、可比较，并保留失败轨迹与采用/拒绝理由。
- 实验室完成不能替代真实 ERP 主线的权限、状态、写入、回读和对账证据。
...
- [x] Frappe 与 Runtime 之间的用户绑定授权方案（2026-08-25 由 `docs/decisions/ADR-0003-runtime-user-authorization.md` 批准，并在 Phase 2 通过真实 HTTP 链路验证）。
- [x] 将已批准的审批基线映射到固定开发 site 的 ERPNext Role/Permission Matrix 与 Workflow 配置（2026-08-27 由 ADR-0007 与只读取证固定；其他企业 site 的更严格 Workflow 仍运行时优先并 fail closed）。
- [ ] 多级审批、金额阈值和策略配置来源。
- [x] Goal 长度、默认公司/仓库/时间范围（2026-08-25 由 P3.1 决策包批准并同步到 Run 契约与前端验收）。
...
- `docs/项目方向纠偏.md` — 2026-08-26 批准的学习定位、同仓双层结构与 Phase 4–13 路线。
